...
const protectedShellTest = runActiveOrganizationE2e ? test : test.skip;

test.describe("authenticated protected shell E2E contract", () => {
  ...
  protectedShellTest("renders the protected app shell", async ({ page }) => {
    ...
    await expect(page.getByRole("link", { name: /FamilyFinance/ })).toBeVisible();
    await expect(page.getByText("Organização").first()).toBeVisible();
