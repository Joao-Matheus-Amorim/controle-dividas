...
const reportsRouteTest = runActiveOrganizationE2e ? test : test.skip;

test.describe("authenticated protected reports route E2E contract", () => {
  ...
  reportsRouteTest("opens the reports route without redirecting away from the protected app", async ({ page }) => {
    ...
    await page.goto("/protected/relatorios");

    await expect(page).toHaveURL(/\/protected\/relatorios(?:\?|$)/, { timeout: 15_000 });
    await expect(page).not.toHaveURL(/\/onboarding\/organizacao/);
