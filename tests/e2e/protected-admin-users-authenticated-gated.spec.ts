...
const adminUsersRouteTest = runAdminE2e ? test : test.skip;

test.describe("authenticated protected admin users route E2E contract", () => {
  ...
  adminUsersRouteTest("opens the admin users route without redirecting away from the protected app", async ({ page }) => {
    ...
    await page.goto("/protected/admin/usuarios");

    await expect(page).toHaveURL(/\/protected\/admin\/usuarios(?:\?|$)/, { timeout: 15_000 });
    await expect(page).not.toHaveURL(/\/onboarding\/organizacao/);
    await expect(page.getByRole("heading", { name: "Usuários" })).toBeVisible();
    await expect(page.getByText("Acessos da família")).toBeVisible();
  });
});
