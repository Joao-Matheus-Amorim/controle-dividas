...
const receivablesRouteTest = runActiveOrganizationE2e ? test : test.skip;

test.describe("authenticated protected receivables route E2E contract", () => {
  ...
  receivablesRouteTest("opens the receivables route without redirecting away from the protected app", async ({ page }) => {
    ...
    await expect(page).toHaveURL(/\/protected\/contas-a-receber(?:\?|$)/, { timeout: 15_000 });
    await expect(page).not.toHaveURL(/\/onboarding\/organizacao/);
    await expect(page.getByRole("heading", { name: "Receber" })).toBeVisible();
    await expect(page.getByText("Entradas e rendas")).toBeVisible();
  });
});