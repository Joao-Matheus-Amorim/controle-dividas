...
const payablesRouteTest = runActiveOrganizationE2e ? test : test.skip;

test.describe("authenticated protected payables route E2E contract", () => {
  ...
  payablesRouteTest("opens the payables route without redirecting away from the protected app", async ({ page }) => {
    ...
    await expect(page).toHaveURL(/\/protected\/contas-a-pagar(?:\?|$)/, { timeout: 15_000 });
    await expect(page).not.toHaveURL(/\/onboarding\/organizacao/);
    await expect(page.getByRole("heading", { name: "Contas e dividas" })).toBeVisible();
    await expect(page.getByText("Contas fixas, avulsas, pagamentos e vencimentos")).toBeVisible();
  });
});
