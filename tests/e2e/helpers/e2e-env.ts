...
const requiredMultiOrgSwitchVariables = [
  "E2E_MULTI_ORG_EMAIL",
...
export function getMultiOrgSwitchE2eConfig(env: Env = process.env) {
  const enabled = env.RUN_MULTI_ORG_SWITCH_E2E === "true";
  const missingVariables = enabled
    ? requiredMultiOrgSwitchVariables.filter((key) => !env[key])
    : [];

  return {
    enabled,
    missingVariables,
    email: env.E2E_MULTI_ORG_EMAIL,
    password: env.E2E_MULTI_ORG_PASSWORD,
    supabaseUrl: env.NEXT_PUBLIC_SUPABASE_URL,
    serviceRoleKey: env.SUPABASE_SERVICE_ROLE_KEY,
  };
}
