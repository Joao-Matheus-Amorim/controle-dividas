...
const adminRouteTest = runAdminE2e ? test : test.skip;

test.describe("authenticated protected admin route E2E contract", () => {
  ...
  adminRouteTest("opens the admin route without redirecting away from the protected app", async ({ page }) => {
    ...
    await expect(page.getByText("Gerenciamento familiar")).toBeVisible();
  });
});
