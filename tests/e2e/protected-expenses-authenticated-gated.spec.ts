...
const expensesRouteTest = runActiveOrganizationE2e ? test : test.skip;

test.describe("authenticated protected expenses route E2E contract", () => {
  ...
  expensesRouteTest("opens the expenses route without redirecting away from the protected app", async ({ page }) => {
    ...
    await expect(page).toHaveURL(/\/protected\/gastos(?:\?|$)/, { timeout: 15_000 });
    await expect(page).not.toHaveURL(/\/onboarding\/organizacao/);
    await expect(page.getByRole("heading", { name: "Gastos" })).toBeVisible();
    await expect(page.getByText("Lançamentos da família")).toBeVisible();
  });
});