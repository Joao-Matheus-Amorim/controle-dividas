    ...
    <section className={canManageSources ? "grid gap-4 xl:grid-cols-[0.9fr_1.1fr]" : "grid gap-4"}>
      {canManageSources ? (
        <div className="rounded-[1.5rem] border border-white/10 bg-white/[0.04] p-4">
          <div className="mb-4 flex items-center justify-between">
            <p className="text-[11px] font-bold uppercase tracking-[0.22em] text-white/25">Nova origem</p>
      ...
      <div className="space-y-3 rounded-[1.5rem] border border-white/10 bg-white/[0.04] p-4">
        <div className="flex items-center justify-between">
          <p className="text-[11px] font-bold uppercase tracking-[0.22em] text-white/25">Origens de recebimento</p>
          <p className="text-xs font-semibold text-[#8b72f8]">{sources.length}</p>
        </div>
        {sources.map((source) => (
          <div key={source.id} className="flex items-start justify-between gap-4 rounded-2xl border border-white/10 bg-[#080810]/50 p-3">
            <div>
              ...
              <p className="mt-1 text-xs text-white/35">{source.description || "Sem descricao"}</p>
            </div>
