...
export function PayableList({
  ...
  return (
    <section className="space-y-4 rounded-[1.5rem] border border-border bg-ff-bg-soft p-4">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          <p className="text-[11px] font-bold uppercase tracking-[0.22em] text-ff-subtle-foreground">Contas e dividas cadastradas</p>
          <p className="mt-1 text-sm text-ff-subtle-foreground">
            {filteredBills.length} de {bills.length} itens visiveis no seu escopo.
          </p>
        </div>
        {hasActiveFilters ? (
          <Link href={clearFiltersHref} className="text-xs font-semibold text-primary underline-offset-4 hover:underline">
            Limpar filtros
          </Link>
        ) : null}
      </div>
      ...
      {bills.length === 0 ? (
        ...
        <AppEmptyState
          ...
          action={
            <Button asChild size="sm" variant="outline" className="h-10 w-full rounded-2xl border-border bg-transparent text-foreground hover:bg-ff-bg-soft sm:w-auto">
