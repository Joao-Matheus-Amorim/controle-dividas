-- Move payable bill writes from authenticated-user ownership to organization-scoped writes.
-- Preconditions:
...
-- - Application actions enforce the same CONTAS_A_PAGAR module/member permissions before writes.
...
drop policy if exists "payable_bills_insert_owner_organization" on public.payable_bills;
drop policy if exists "payable_bills_update_owner_organization" on public.payable_bills;
drop policy if exists "payable_bills_delete_owner_organization" on public.payable_bills;

create policy "payable_bills_insert_organization"
  on public.payable_bills
  for insert
  with check (
    public.can_manage_organization_payable_bill(organization_id, responsible_member_id, 'can_create')
...
create policy "payable_bills_update_organization"
  ...
  for update
  using (
    public.can_manage_organization_payable_bill(organization_id, responsible_member_id, 'can_edit')
  )
  with check (
    public.can_manage_organization_payable_bill(organization_id, responsible_member_id, 'can_edit')
    and public.organization_legacy_owner_matches(organization_id, owner_id)
...
create policy "payable_bills_delete_organization"
  ...
  for delete
  using (
    public.can_manage_organization_payable_bill(organization_id, responsible_member_id, 'can_delete')
  );
