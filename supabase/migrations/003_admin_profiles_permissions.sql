...
-- Projeto personalizado para uma unica familia, nao SaaS.
...
create policy "profiles_delete_family"
on public.profiles
for delete
using (owner_id = auth.uid());

create policy "permissions_select_family"
on public.user_module_permissions
for select
using (owner_id = auth.uid());

create policy "permissions_insert_family"
on public.user_module_permissions
for insert
with check (owner_id = auth.uid());

create policy "permissions_update_family"
on public.user_module_permissions
for update
using (owner_id = auth.uid())
with check (owner_id = auth.uid());

create policy "permissions_delete_family"
on public.user_module_permissions
for delete
using (owner_id = auth.uid());
