-- Make paid/received status RPCs idempotent when a ledger movement already
-- exists. This preserves recovery for rows that were reverted before estorno
-- exists: setting the status back does not duplicate the movement or balance.
