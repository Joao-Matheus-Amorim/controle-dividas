-- Fix admin invitation token acceptance on Supabase projects where pgcrypto lives
-- in the extensions schema instead of public.
...
begin
  ...
  hashed_token := encode(digest(convert_to(normalized_token, 'UTF8'), 'sha256'::text), 'hex');
