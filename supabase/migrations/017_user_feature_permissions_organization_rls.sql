...
-- Adds organization-aware RLS policies for user_feature_permissions only.
--
-- Transitional rule:
...
-- - remove owner_id;
-- - change profiles;
-- - change user_module_permissions;
...
create policy "feature_permissions_select_organization_or_legacy"
...
create policy "feature_permissions_insert_owner_organization_or_legacy"
on public.user_feature_permissions
for insert
with check (
  owner_id = auth.uid()
...
create policy "feature_permissions_update_owner_organization_or_legacy"
on public.user_feature_permissions
for update
using (
  owner_id = auth.uid()
...
create policy "feature_permissions_delete_owner_organization_or_legacy"
on public.user_feature_permissions
for delete
using (
  owner_id = auth.uid()
