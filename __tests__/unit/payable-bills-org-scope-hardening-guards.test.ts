...
const migrationPath = "supabase/migrations/023_payable_bills_organization_scope_hardening.sql";
const runbookPath = "docs/runbooks/PAYABLE_BILLS_ORG_SCOPE_HARDENING.md";
...
describe("payable bills organization scope hardening", () => {
  ...
  it("targets only payable_bills organization_id", () => {
    expect(executableMigration).toContain("from public.payable_bills");
    expect(executableMigration).toContain("alter table public.payable_bills");
  ...
  it("fails before hardening if null organization rows still exist", () => {
    ...
    const constraintPosition = executableMigration.indexOf("alter table public.payable_bills");
  ...
  it("documents required checks and rollback expectation", () => {
    expect(runbook).toContain("supabase/migrations/023_payable_bills_organization_scope_hardening.sql");
    expect(runbook).toContain("docs/sql/payable-bills-organization-null-preflight.sql");
    expect(runbook).toContain("docs/sql/payable-bills-organization-dry-run.sql");
