...
const migrationPath = "supabase/migrations/026_user_module_permissions_organization_scope_hardening.sql";
const runbookPath = "docs/runbooks/USER_MODULE_PERMISSIONS_ORG_SCOPE_HARDENING.md";
...
describe("user module permissions organization scope hardening", () => {
  ...
  it("targets only user_module_permissions organization_id", () => {
    expect(migration).toContain('from "public"."user_module_permissions"');
    expect(migration).toContain('alter table "public"."user_module_permissions"');
  ...
  it("fails before hardening if null organization rows still exist", () => {
    ...
    expect(migration.indexOf('alter table "public"."user_module_permissions"')).toBeGreaterThan(
  ...
  it("documents required checks and rollback expectation", () => {
    ...
    expect(runbook).toContain("docs/sql/module-permissions-organization-null-preflight.sql");
    expect(runbook).toContain("docs/sql/module-permissions-organization-dry-run.sql");
