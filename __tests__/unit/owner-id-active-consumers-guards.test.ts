...
describe("owner_id active consumers inventory guards", () => {
  ...
  it("records Admin as the active owner-based runtime exception", () => {
    ...
    expect(inventory).toContain("read/write path estao organization-first com gate admin por organizacao ativa");
    ...
    expect(inventory).toContain("esse contrato ainda bloqueia access-control e schema final");
