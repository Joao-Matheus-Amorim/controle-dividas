...
const migrationPath = "supabase/migrations/022_expenses_organization_scope_hardening.sql";
const runbookPath = "docs/runbooks/EXPENSES_ORG_SCOPE_HARDENING.md";
...
describe("expenses organization scope hardening", () => {
  ...
  it("targets only expenses organization_id", () => {
    expect(executableMigration).toContain("from public.expenses");
    expect(executableMigration).toContain("alter table public.expenses");
  ...
  it("fails before hardening if null organization rows still exist", () => {
    ...
    const constraintPosition = executableMigration.indexOf("alter table public.expenses");
  ...
  it("documents required checks and rollback expectation", () => {
    expect(runbook).toContain("supabase/migrations/022_expenses_organization_scope_hardening.sql");
    expect(runbook).toContain("docs/sql/expenses-organization-null-preflight.sql");
    expect(runbook).toContain("docs/sql/expenses-organization-backfill-dry-run.sql");
