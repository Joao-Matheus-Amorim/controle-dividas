...
const migrationPath = "supabase/migrations/028_profiles_organization_scope_hardening.sql";
const runbookPath = "docs/runbooks/PROFILES_ORG_SCOPE_HARDENING.md";
...
describe("profiles organization scope hardening", () => {
  ...
  it("targets only profiles organization_id", () => {
  ...
  it("fails before hardening if null organization rows still exist", () => {
    ...
    expect(migration.indexOf('alter table "public"."profiles"')).toBeGreaterThan(migration.indexOf("if exists"));
  ...
  it("documents required checks and rollback expectation", () => {
    ...
    expect(runbook).toContain("docs/sql/profile-organization-null-check.sql");
    expect(runbook).toContain("docs/sql/profile-organization-dry-run.sql");
    expect(runbook).toContain("null_organization_rows = 0");
    expect(runbook).toContain("total_legacy_null_organization_rows = 0");
