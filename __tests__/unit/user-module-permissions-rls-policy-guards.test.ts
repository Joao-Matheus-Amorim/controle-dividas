...
const migration = readFileSync(
  join(process.cwd(), "supabase/migrations/037_user_module_permissions_rls_remove_legacy_fallback.sql"),
  "utf8",
);
...
describe("user_module_permissions RLS policies", () => {
  ...
  it("removes legacy null-organization fallback from module permission policies", () => {
    ...
    expect(executablePolicySql).toContain("module_permissions_select_organization");
    expect(executablePolicySql).toContain("module_permissions_insert_owner_organization");
    expect(executablePolicySql).toContain("module_permissions_update_owner_organization");
    expect(executablePolicySql).toContain("module_permissions_delete_owner_organization");
  });
});
