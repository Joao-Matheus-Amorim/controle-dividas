...
describe("receivable income write audit runtime guards", () => {
  const actions = read("app/protected/contas-a-receber/actions.ts");
  const createAction = functionBlock(actions, "createreceivableincome");
  const updateAction = functionBlock(actions, "updatereceivableincome");
  ...
  it("records receivable create and update events through audit and rate limit boundaries", () => {
    expect(actions).toContain("recordreceivableincomeauditevent");
    expect(actions).toContain("finance.receivable.create");
    expect(actions).toContain("finance.receivable.update");
    expect(actions).toContain('targettype: "receivable_income"');
    expect(actions).toContain('operationkey: "finance.receivable.create"');
    expect(actions).toContain('operationkey: "finance.receivable.update"');

    expect(createAction).toContain("receivablecreateratelimit");
    ...
    expect(createAction).toContain('.from("receivable_incomes").insert({');
    expect(createAction).toContain('.select("id").single()');
    expect(createAction).toContain("receivable_created");

    expect(updateAction).toContain("const incomechanged");
    expect(updateAction).toContain("receivableupdateratelimit");
    ...
    expect(updateAction).toContain("receivable_changed");
  ...
  it("keeps emitted metadata redacted and small", () => {
    expect(actions).toContain("receivable_created");
    expect(actions).toContain("receivable_changed");
    ...
    expect(actions).not.toContain("previous_source");
    expect(actions).not.toContain("next_source");
  ...
  it("keeps docs aligned with receivable write runtime coverage", () => {
    for (const source of [schemaPlan, rateLimitPlan, contract, roadmap, liveStatus, gapRegister]) {
      expect(source).toContain("finance.receivable.create");
      expect(source).toContain("finance.receivable.update");
    }

    for (const source of [rateLimitPlan, contract, roadmap, liveStatus, gapRegister]) {
      expect(source).toContain("receivable write rate limit runtime");
    }

    for (const source of [schemaPlan, roadmap, liveStatus, gapRegister]) {
      expect(source).toContain("receivable write audit runtime");
    }
  });
});
