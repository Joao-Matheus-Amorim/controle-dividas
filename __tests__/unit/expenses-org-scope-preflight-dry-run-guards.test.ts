...
const sqlFiles = {
  preflight: "docs/sql/expenses-organization-null-preflight.sql",
  dryRun: "docs/sql/expenses-organization-backfill-dry-run.sql",
} as const;
...
describe("expenses organization scope preflight and dry-run SQL", () => {
  ...
  it("keeps the expenses null preflight read-only and table-scoped", () => {
    expect(preflight).toContain("issue: #586");
    expect(preflight).toContain("read-only");
    expect(preflight).toContain("from public.expenses");
  ...
  it("keeps the expenses dry-run read-only and table-scoped", () => {
    expect(dryRun).toContain("issue: #586");
    expect(dryRun).toContain("read-only");
    expect(dryRun).toContain("from public.expenses");
