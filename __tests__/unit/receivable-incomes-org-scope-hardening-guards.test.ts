...
const migrationPath = "supabase/migrations/024_receivable_incomes_organization_scope_hardening.sql";
const runbookPath = "docs/runbooks/RECEIVABLE_INCOMES_ORG_SCOPE_HARDENING.md";
...
describe("receivable incomes organization scope hardening", () => {
  ...
  it("targets only receivable_incomes organization_id", () => {
    expect(migration).toContain("from public.receivable_incomes");
    expect(migration).toContain("alter table public.receivable_incomes");
    ...
    expect(migration).not.toContain("public.banks");
  });
  ...
  it("fails before hardening if null organization rows still exist", () => {
    ...
    expect(migration.indexOf("alter table public.receivable_incomes")).toBeGreaterThan(
  ...
  it("documents required checks and rollback expectation", () => {
    ...
    expect(runbook).toContain("docs/sql/receivable-incomes-organization-null-preflight.sql");
    expect(runbook).toContain("docs/sql/receivable-incomes-organization-dry-run.sql");
