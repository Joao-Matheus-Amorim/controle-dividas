...
describe("user_feature_permissions RLS gated integration", () => {
  ...
  rlsIt(
    "returns only feature permissions for the active organization",
    async () => {
      ...
      try {
        ...
        const profileResult = await admin.from("profiles").insert([
          ...
          },
          {
            ...
            owner_id: ownerId,
            organization_id: orgA.id,
            name: `${fixture.prefix} Feature Profile A2`,
            email: `${fixture.slugPrefix}+feature-profile-a2@example.com`,
            role: "user",
            is_active: true,
          },
        ]);
