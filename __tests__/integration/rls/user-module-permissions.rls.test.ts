...
describe("user_module_permissions RLS gated integration", () => {
  ...
  rlsIt("returns only module permissions for the active organization plus legacy rows", async () => {
    ...
    try {
      ...
      const permissionResult = await admin.from("user_module_permissions").insert([
        {
          id: permissionAId,
          owner_id: ownerId,
          organization_id: orgA.id,
      ...
      await admin.from("user_module_permissions").delete().in("id", permissionIds);
