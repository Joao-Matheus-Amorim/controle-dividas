...
| `docs/audits/OWNER_ID_RETIREMENT_INVENTORY_2026-06-01.md` | Atual | Inventario atual do G-005 para planejar retirada futura de `owner_id`. | Mantem `owner_id` como aberto controlado; Admin/access-control, categorias, Pessoas/family_members e Bancos ja tem paths organization-first com owner legado transicional; `family_members` e `banks` write RLS tambem validam o owner legado da organizacao alvo, e `banks` exige permissao BANCOS por membro/acao; schema e demais consumidores seguem transicionais. |
| `docs/audits/OWNER_ID_ACTIVE_CONSUMERS_2026-06-01.md` | Atual | Inventario dos consumidores ativos dos helpers owner-only. | Identifica Admin como excecao ativa, registra Categorias, Pessoas/family_members e Bancos como consumidores reduzidos para organization-first read/write, registra constraint DB do owner legado em `family_members` e `banks` com permissao BANCOS para bancos, e bloqueia retorno das telas financeiras para helpers legados. |
