...
| `docs/audits/ADMIN_INVITATION_BOOTSTRAP_CONTRACT.md` | Atual como contrato com schema/preflight e runtime parcial | Contrato para substituir futuramente `ADMIN_EMAIL` por convite/admin por organizacao; `supabase/migrations/044_admin_invitations_schema.sql` cria armazenamento/RLS iniciais e `app/protected/admin/invitation-actions.ts` cobre criar, revogar e preparar reenvio; `supabase/migrations/045_accept_admin_invitation_rpc.sql` e `app/auth/convite/actions.ts` cobrem aceite/linking. | Email delivery, UI, cron de expiracao e runtime final seguem pendentes; bloqueia remocao de `ADMIN_EMAIL` e `owner_id` ate convites, audit, rate limit e rollback existirem. |
| `docs/audits/ADMIN_INVITATION_DELIVERY_UI_CONTRACT.md` | Atual como contrato pre-runtime | Contrato para delivery e UI de convite admin sem armazenar, logar ou expor token bruto. | Provider de email, delivery runtime, UI de aceite e cron seguem pendentes; usar antes de qualquer dependencia/provider ou tela `/auth/convite`. |
