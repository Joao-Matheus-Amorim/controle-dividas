...
| `docs/audits/ADMIN_INVITATION_BOOTSTRAP_CONTRACT.md` | Atual como contrato pre-runtime com schema/preflight versionado | Contrato para substituir futuramente `ADMIN_EMAIL` por convite/admin por organizacao; `supabase/migrations/044_admin_invitations_schema.sql` cria armazenamento e RLS iniciais. | Runtime de convite/admin segue pendente; bloqueia remocao de `ADMIN_EMAIL` e `owner_id` ate convites, audit, rate limit e rollback existirem. |
