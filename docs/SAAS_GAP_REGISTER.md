...
| --- | --- | --- | --- |
| GAP-005 | Compatibility | `owner_id` remains part of the transitional model; admin read/write paths in `lib/finance/admin-server.ts` and `app/protected/admin/actions.ts` now require admin rights in the active organization and are organization-first; `lib/finance/access-control.ts` also resolves profile, permissions and accessible members by active organization; people, category, bank, expense, payable and receivable read helpers can resolve by active organization; category settings write/actions plus `expense_categories` write RLS are scoped to owner/admin category managers; people write/actions plus `family_members` write RLS are scoped to owner/admin people managers, with `family_members` RLS also constraining written rows to the target organization's legacy owner id; bank actions preserve BANCOS permissions, remove owner filters, write the organization's legacy owner id, and `banks` write RLS enforces BANCOS permission by member/action plus constrains that owner id to the target organization; expense actions preserve GASTOS permissions, remove owner filters, write the organization's legacy owner id, and `expenses` write RLS enforces GASTOS permission by member/action plus constrains that owner id to the target organization and validates member/category tenant ownership; payable actions preserve CONTAS_A_PAGAR permissions, remove owner filters, write the organization's legacy owner id, and `payable_bills` write RLS enforces CONTAS_A_PAGAR permission by member/action plus constrains that owner id to the target organization and validates responsible member tenant ownership; receivable actions preserve CONTAS_A_RECEBER permissions, remove owner filters, write the organization's legacy owner id, and `receivable_incomes` write RLS enforces CONTAS_A_RECEBER permission by member/action plus constrains that owner id to the target organization and validates receiver member tenant ownership; new categories, family members, banks, expenses, payable bills and receivable incomes still write the organization's legacy owner id for compatibility; legacy seed/bootstrap and the legacy finance facade reads now use the active organization's legacy owner id plus `organization_id` instead of the authenticated user's id; schema and other consumers still preserve owner compatibility. | Continue with the next dedicated owner_id consumer before schema retirement. |
...
| GAP-020 | AI finance intake | AI finance intake contract exists in `docs/audits/AI_FINANCE_INTAKE_CONTRACT.md`, defining allowed intents, required fields, option catalogs, member scope rules, missing-field questions and review-only save boundaries. `docs/audits/AI_FINANCE_PROVIDER_ENDPOINT_CONTRACT.md` defines the provider/endpoint pre-runtime boundary. `lib/finance/ai-finance-intake-schema.ts` provides the first structured draft schema and catalog validator; `lib/finance/ai-finance-intake-catalogs.ts` assembles server-side active-organization catalogs by intent, create permissions, controlled bank names, account types and currencies; `lib/finance/ai-finance-intake-runtime.ts` provides the first server-only review-only boundary with `provider: none`, `canAutoSave: false` and no direct save action; `lib/finance/ai-finance-provider-config.ts` provides the fail-closed provider configuration boundary, disabled by default; `components/finance/assisted-draft-review-boundary.tsx` centralizes the review-only UI boundary for assisted drafts. Current assisted drafts remain deterministic and review-only; there is no model, endpoint, provider dependency, RLS or direct save runtime. | Implement model-backed intake only in a dedicated provider/endpoint runtime PR with fail-closed config, rate limit, audit and rollback. |

GAP-020 operational note: `docs/audits/AI_COPILOT_ROADMAP.md` is now the living roadmap for the broader AI copilot feature. The current runtime includes `/api/ai` as an initial read-only endpoint with guarded actions (`getDashboardSummary`, `getUpcomingBills`) and `ai_actions` audit logging. Chat/copilot UI, provider/model runtime, prompt-backed draft generation, embeddings, memory and any direct write action remain future work and must follow the roadmap blocks.
...
Member write audit/rate limit runtime includes `finance.member.delete` for guarded family member deletion.
...
GAP-020 now has an AI finance intake contract, a provider/endpoint pre-runtime contract, an AI copilot roadmap, an initial structured draft schema, server-side catalog assembly, a server-only review-only boundary in `lib/finance/ai-finance-intake-runtime.ts`, a fail-closed provider configuration boundary in `lib/finance/ai-finance-provider-config.ts`, a shared review-only UI boundary in `components/finance/assisted-draft-review-boundary.tsx`, and `/api/ai` as a guarded read-only endpoint. Model-backed runtime, chat/copilot UI, provider dependency and direct-save behavior remain future work. Existing assisted drafts are useful review-only helpers and must not be treated as provider-backed IA.
