...
| GAP-006 | Billing | Billing has a local plan contract, billing settings status UI, subscription flow contract, Stripe configuration boundary, and Stripe checkout runtime is implemented. Portal, webhook, subscriptions, and commercial enforcement are not implemented. | Implement webhook and portal in dedicated PRs after checkout evidence, keeping commercial enforcement separated. |
...
GAP-006 remains a product implementation risk after evidence gates: billing status UI, subscription flow contract, Stripe configuration boundary, and checkout runtime exist, but webhook, portal, subscription state sync, and commercial enforcement are still not implemented.
