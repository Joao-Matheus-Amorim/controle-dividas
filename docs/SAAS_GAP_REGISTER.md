...
| --- | --- | --- | --- |
| GAP-005 | Compatibility | `owner_id` remains part of the transitional model; admin read/write paths in `lib/finance/admin-server.ts` and `app/protected/admin/actions.ts` now require admin rights in the active organization and are organization-first; `lib/finance/access-control.ts` also resolves permissions and accessible members by active organization; people and category read helpers can resolve by active organization; category settings write/actions and `expense_categories` write RLS are scoped to owner/admin category managers while new categories still write the organization's legacy owner id for compatibility; schema and other consumers still preserve owner compatibility. | Continue with the next dedicated owner_id consumer before schema retirement. |
