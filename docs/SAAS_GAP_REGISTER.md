...
| --- | --- | --- | --- |
| GAP-005 | Compatibility | `owner_id` remains part of the transitional model; admin read path in `lib/finance/admin-server.ts` now requires admin rights in the active organization and is organization-first, while admin writes, access-control, and schema still preserve owner compatibility. | Continue with write path admin organization-first before access-control or schema retirement. |
