...
| GAP-016 | Onboarding terminology | Onboarding terminology contract exists in `docs/audits/ONBOARDING_TERMINOLOGY_CONTRACT.md`, defining user-facing wording, technical terminology boundaries, current copy inventory, route boundaries, and acceptance criteria before UI changes. No runtime, UI, route, schema, RLS, billing, or dependency change is implemented. | Use the contract before changing onboarding copy or UX; update it when the first runtime copy change lands. |
...
GAP-016 now has an onboarding terminology contract, but runtime copy adoption remains future work.
