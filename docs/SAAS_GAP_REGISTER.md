...
GAP-020 now has an AI finance intake contract, a provider/endpoint pre-runtime contract, an initial structured draft schema, server-side catalog assembly, a server-only review-only boundary in `lib/finance/ai-finance-intake-runtime.ts`, a fail-closed provider configuration boundary in `lib/finance/ai-finance-provider-config.ts`, and a shared review-only UI boundary in `components/finance/assisted-draft-review-boundary.tsx`, but model-backed runtime remains future work. Existing assisted drafts are useful review-only helpers and must not be treated as provider-backed IA.
