...
GAP-020 now has an AI finance intake contract, an initial structured draft schema, server-side catalog assembly, and a server-only review-only boundary in `lib/finance/ai-finance-intake-runtime.ts`, but model-backed runtime remains future work. Existing assisted drafts are useful review-only helpers and must not be treated as provider-backed IA.
