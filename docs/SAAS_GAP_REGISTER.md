...
| GAP-006 | Billing | Billing has a local plan contract, billing settings status UI is implemented, the subscription flow contract is documented, and the Stripe configuration boundary is implemented, but Stripe checkout, portal, webhook, subscriptions, and commercial enforcement are not implemented. | Implement checkout runtime in a dedicated PR, keeping webhook and portal separated. |
...
GAP-006 is the next product implementation risk after evidence gates: billing status UI, subscription flow contract, and Stripe configuration boundary exist, but subscription runtime is still not implemented.
