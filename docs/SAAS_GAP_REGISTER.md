...
| GAP-020 | AI finance intake | AI finance intake contract exists in `docs/audits/AI_FINANCE_INTAKE_CONTRACT.md`, defining allowed intents, required fields, option catalogs, member scope rules, missing-field questions and review-only save boundaries. Current assisted drafts remain deterministic and review-only; there is no model, endpoint, provider, API key, schema, RLS or direct save runtime. | Implement model-backed intake only after structured response schema, organization option catalogs, server validation and review-only UI boundaries are added in dedicated PRs. |
...
GAP-020 now has an AI finance intake contract, but model-backed runtime remains future work. Existing assisted drafts are useful review-only helpers and must not be treated as provider-backed IA.
