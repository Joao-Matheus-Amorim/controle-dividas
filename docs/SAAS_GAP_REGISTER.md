...
| GAP-007 | Admin bootstrap | `ADMIN_EMAIL` remains a bootstrap mechanism; the final organization admin and invitation contract exists in `docs/audits/ADMIN_INVITATION_BOOTSTRAP_CONTRACT.md`; schema/preflight is versioned in `supabase/migrations/044_admin_invitations_schema.sql`; create/revoke/resend runtime is versioned in `app/protected/admin/invitation-actions.ts`; and acceptance/linking runtime is versioned in `supabase/migrations/045_accept_admin_invitation_rpc.sql` plus `app/auth/convite/actions.ts` with audit and rate limit. Email delivery, UI, cron expiry, and `ADMIN_EMAIL` removal are not implemented. | Implement invitation delivery/UI and expiry cleanup in dedicated PRs before removing `ADMIN_EMAIL`. |
...
| GAP-014 | Membership lifecycle | The one-active-membership database limit was removed; final invitation/admin membership behavior is defined by `docs/audits/ADMIN_INVITATION_BOOTSTRAP_CONTRACT.md`; invitation storage preflight, create/revoke/resend runtime, and acceptance/linking runtime exist. Email delivery, UI, cron expiry, and `ADMIN_EMAIL` removal remain pending. | Implement invitation delivery/UI and expiry cleanup before removing transitional bootstrap assumptions. |
