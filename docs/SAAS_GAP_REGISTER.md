...
| GAP-020 | AI finance intake | AI finance intake contract exists in `docs/audits/AI_FINANCE_INTAKE_CONTRACT.md`, defining allowed intents, required fields, option catalogs, member scope rules, missing-field questions and review-only save boundaries. `lib/finance/ai-finance-intake-schema.ts` now provides the first structured draft schema and catalog validator. Current assisted drafts remain deterministic and review-only; there is no model, endpoint, provider, API key, RLS or direct save runtime. | Implement model-backed intake only after organization option catalog assembly, server-side runtime boundary and review-only UI boundaries are added in dedicated PRs. |
...
GAP-020 now has an AI finance intake contract and an initial structured draft schema, but model-backed runtime remains future work. Existing assisted drafts are useful review-only helpers and must not be treated as provider-backed IA.
