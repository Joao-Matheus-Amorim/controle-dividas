...
| GAP-007 | Admin bootstrap | `ADMIN_EMAIL` remains a bootstrap mechanism; the final organization admin and invitation contract exists in `docs/audits/ADMIN_INVITATION_BOOTSTRAP_CONTRACT.md`; schema/preflight is versioned in `supabase/migrations/044_admin_invitations_schema.sql`, and create/revoke/resend runtime is versioned in `app/protected/admin/invitation-actions.ts` with audit and rate limit. Email delivery, acceptance, and `ADMIN_EMAIL` removal are not implemented. | Implement invitation acceptance/linking runtime in a dedicated PR before removing `ADMIN_EMAIL`. |
