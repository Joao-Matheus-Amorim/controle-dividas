...
| --- | --- | --- | --- |
| GAP-005 | Compatibility | `owner_id` remains part of the transitional model; admin read/write paths in `lib/finance/admin-server.ts` and `app/protected/admin/actions.ts` now require admin rights in the active organization and are organization-first; `lib/finance/access-control.ts` also resolves permissions and accessible members by active organization; people, category and bank read helpers can resolve by active organization; category settings write/actions plus `expense_categories` write RLS are scoped to owner/admin category managers; people write/actions plus `family_members` write RLS are scoped to owner/admin people managers, with `family_members` RLS also constraining written rows to the target organization's legacy owner id; bank actions preserve BANCOS permissions, remove owner filters, write the organization's legacy owner id, and `banks` write RLS enforces BANCOS permission by member/action plus constrains that owner id to the target organization; new categories, family members and banks still write the organization's legacy owner id for compatibility; schema and other consumers still preserve owner compatibility. | Continue with the next dedicated owner_id consumer before schema retirement. |
