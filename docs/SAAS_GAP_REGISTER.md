...
GAP-014 through GAP-019 came from external review and must be handled as separate issue/PR cycles before implementation. GAP-015 now has a planning contract, audit event schema/read-side RLS, audit event write boundary, billing checkout audit runtime, billing checkout rate limit runtime, expense delete rate limit runtime, payable delete rate limit runtime, payable status rate limit runtime, receivable delete rate limit runtime, receivable status rate limit runtime, bank delete rate limit runtime, bank balance rate limit runtime, member limit audit runtime, member limit rate limit runtime, member status audit runtime, member status rate limit runtime, member write audit runtime, member write rate limit runtime, category delete rate limit runtime, admin permission audit runtime, admin permission rate limit runtime, admin user audit runtime, admin user rate limit runtime, payable bill audit runtime, receivable income audit runtime, expense audit runtime, category delete audit runtime, bank audit runtime, rate limit plan, and data retention plan, audit event retention preflight, and audit event retention cleanup runtime, but it remains open until broader rate limiting and data retention cleanup controls are implemented in dedicated PRs.
...
- GAP-015 tracks controls expected by users handling sensitive financial data; billing checkout audit, billing checkout rate limiting, expense delete rate limiting, payable delete rate limiting, payable status rate limiting, receivable delete rate limiting, receivable status rate limiting, bank delete rate limiting, bank balance rate limiting, member limit audit and rate limiting, member status audit and rate limiting, member write audit and rate limiting, category delete rate limiting, admin permission audit and rate limiting, admin user audit and rate limiting, payable bill, receivable income, expense, category delete, and bank audit runtime exist, but broader rate limiting and retention cleanup runtime are still pending.
