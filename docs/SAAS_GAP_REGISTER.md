...
| --- | --- | --- | --- |
| GAP-005 | Compatibility | `owner_id` remains part of the transitional model; admin read/write paths in `lib/finance/admin-server.ts` and `app/protected/admin/actions.ts` now require admin rights in the active organization and are organization-first; `lib/finance/access-control.ts` also resolves permissions and accessible members by active organization; people read paths in `lib/organizations/people.ts` and `features/protected-pages/pessoas-page.tsx` are organization-first, while `app/protected/pessoas/actions.ts` writes remain owner-compatible with current `family_members` RLS; schema and other consumers still preserve owner compatibility. | Continue with the next dedicated owner_id consumer before schema retirement. |
