...
| GAP-006 | Billing | Billing has a local plan contract, billing settings status UI, subscription flow contract, Stripe configuration boundary, and Stripe checkout runtime is implemented. Real Stripe checkout evidence is pending because there is no Stripe test account/credentials configured yet. Portal, webhook, subscriptions, and commercial enforcement are not implemented. | Create/configure a Stripe test account and credentials, validate checkout runtime evidence, then implement webhook and portal in dedicated PRs. |
