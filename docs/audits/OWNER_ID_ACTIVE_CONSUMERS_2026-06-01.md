...
| Seeds/bootstrap | `lib/finance/server.ts` chama `seedInitialFinanceDataForOwner` | ativo na criacao inicial, mas grava o `owner_id` legado a partir de `organization.owner_auth_user_id` da organizacao ativa; as leituras legadas de membros/categorias no mesmo facade filtram pelo mesmo owner legado e pelo `organization_id` da organizacao ativa | manter ate schema final |
