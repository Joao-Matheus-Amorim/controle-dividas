...
| Gastos | `features/protected-pages/gastos-page.tsx`, `lib/organizations/expenses.ts`, `app/protected/gastos/actions.ts`, `supabase/migrations/052_expenses_organization_write_rls.sql` | consumidor reduzido: helper e pagina usam `organization_id`; actions removem filtros `profile.owner_id`, preservam permissoes `GASTOS`, create grava `owner_id` como owner legado da organizacao e write RLS valida permissao `GASTOS` por membro/acao mais esse owner legado e as relacoes de membro/categoria na organizacao alvo | manter `owner_id` ate schema final e nao aplicar esse padrao aos outros dominios sem migration/RLS dedicada |
| Contas a pagar | `features/protected-pages/contas-a-pagar-page.tsx`, `lib/organizations/payables.ts`, `app/protected/contas-a-pagar/actions.ts`, `supabase/migrations/053_payable_bills_organization_write_rls.sql` | consumidor reduzido: helper e pagina usam `organization_id`; actions removem filtros `profile.owner_id`, preservam permissoes `CONTAS_A_PAGAR`, create grava `owner_id` como owner legado da organizacao e write RLS valida permissao `CONTAS_A_PAGAR` por membro/acao mais esse owner legado e a relacao do membro responsavel na organizacao alvo | manter `owner_id` ate schema final e nao aplicar esse padrao aos outros dominios sem migration/RLS dedicada |
| Contas a receber | `features/protected-pages/contas-a-receber-page.tsx`, `lib/organizations/receivables.ts`, `app/protected/contas-a-receber/actions.ts`, `supabase/migrations/054_receivable_incomes_organization_write_rls.sql` | consumidor reduzido: helper e pagina usam `organization_id`; actions removem filtros `profile.owner_id`, preservam permissoes `CONTAS_A_RECEBER`, create grava `owner_id` como owner legado da organizacao e write RLS valida permissao `CONTAS_A_RECEBER` por membro/acao mais esse owner legado e a relacao do membro recebedor na organizacao alvo | manter `owner_id` ate schema final e nao aplicar esse padrao aos outros dominios sem migration/RLS dedicada |
| Bancos | `features/protected-pages/bancos-page.tsx`, `lib/organizations/banks.ts`, `app/protected/bancos/actions.ts`, `supabase/migrations/051_banks_organization_write_rls.sql` | consumidor reduzido: helper e pagina usam `organization_id`; actions removem filtros `profile.owner_id`, preservam permissoes `BANCOS`, create grava `owner_id` como owner legado da organizacao e write RLS valida permissao `BANCOS` por membro/acao mais esse owner legado | manter `owner_id` ate schema final e nao aplicar esse padrao aos outros dominios sem migration/RLS dedicada |
...
| Pessoas | `lib/organizations/people.ts`, `features/protected-pages/pessoas-page.tsx`, `app/protected/pessoas/actions.ts`, `supabase/migrations/049_family_members_organization_write_rls.sql`, `supabase/migrations/050_family_members_legacy_owner_write_constraint.sql` | consumidor reduzido: helper e pagina usam `organization_id`; create/edit/status/limit controls e actions exigem owner/admin; write RLS de `family_members` e organization-admin-scoped; DB tambem exige que `owner_id` legado corresponda ao owner da organizacao alvo | manter `owner_id` ate schema final e nao aplicar esse padrao aos outros dominios sem migration/RLS dedicada |
...
| Seeds/bootstrap | `lib/finance/server.ts` chama `seedInitialFinanceDataForOwner` | ativo na criacao inicial, mas grava o `owner_id` legado a partir de `organization.owner_auth_user_id` da organizacao ativa | manter ate schema final |
