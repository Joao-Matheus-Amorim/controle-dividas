...
| Categorias | `lib/organizations/categories.ts`, `features/protected-pages/configuracoes-page.tsx` | consumidor reduzido nos reads: helper org-wide usa `organization_id`; Configuracoes usa lista manejavel por `owner_id = auth.uid()` enquanto edit/delete seguem owner-compatible com a RLS atual de `expense_categories` | criar migration/RLS organization-aware antes de expor categorias org-wide nos controles de edit/delete |
