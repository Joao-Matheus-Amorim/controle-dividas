...
| Actions financeiras | `app/protected/gastos/actions.ts`, `contas-a-pagar`, `contas-a-receber`, `bancos`, `pessoas`, `configuracoes` | category settings write/actions ja sao organization-scoped e novos registros usam `owner_id` legado da organizacao; `pessoas` ja usa reads organization-first, mas writes seguem owner-compatible com a RLS atual de `family_members`; demais actions ainda usam `owner_id` com contexto de organizacao ativa | migrar um dominio por PR apos preflight |
