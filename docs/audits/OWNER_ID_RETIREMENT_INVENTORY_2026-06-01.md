...
| Actions financeiras | `app/protected/gastos/actions.ts`, `contas-a-pagar`, `contas-a-receber`, `bancos`, `pessoas`, `configuracoes` | category settings, people/family_members, banks, expenses/gastos e contas-a-pagar write actions ja sao organization-scoped; `family_members`, `banks`, `expenses` e `payable_bills` write RLS tambem validam que o `owner_id` legado corresponde ao owner da organizacao alvo | migrar um dominio por PR apos preflight |
| Helpers organization-aware | `lib/organizations/*` | `lib/organizations/people.ts`, `lib/organizations/categories.ts`, `lib/organizations/banks.ts`, `lib/organizations/expenses.ts` e `lib/organizations/payables.ts` ja leem por organizacao; contas a receber ainda filtra `owner_id`, mas combina com organizacao | manter ate substituto organization-only ser provado |
...
```txt
G-005 permanece aberto controlado; read/write path admin, access-control, categorias, pessoas/family_members, bancos, gastos e contas-a-pagar organization-first estao versionados, mas seeds, contas a receber e schema final seguem transicionais.
```
