...
Audit event retention preflight runtime exists for organization-scoped `audit_events` older than 365 days, with no cleanup job and no destructive deletion.
No UI change.
...
| Audit event retention | `app/protected/configuracoes/audit-retention-actions.ts` | Preflight-only count for organization-scoped `audit_events` older than 365 days; no cleanup, anonymization, or destructive deletion exists. |
