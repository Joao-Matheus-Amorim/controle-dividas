...
| --- | --- | --- |
| Auth and session flows | login, signup, password reset, password update | Login password, signup authorized email, auth confirm, and password reset request rate limit runtime exist for public auth entry/recovery; remaining auth/session flows still need rate limits and audit outcome model. |
