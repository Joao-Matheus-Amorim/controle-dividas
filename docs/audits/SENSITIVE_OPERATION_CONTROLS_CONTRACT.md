...
| --- | --- | --- |
| Auth and session flows | login, signup, password reset, password update | Signup authorized email, auth confirm, and password reset request rate limit runtime exist for public auth preflight/verification/recovery; remaining auth/session flows still need rate limits and audit outcome model. |
