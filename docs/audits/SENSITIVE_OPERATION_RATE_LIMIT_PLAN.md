...
| --- | --- | --- |
| Public auth | `auth.signup.authorized_email.check`, `auth.confirm.verify`; login, signup, password reset still pending | Highest abuse risk; actor may be anonymous. Runtime public-auth boundaries use organization key `public-auth`; signup authorized email preflight keys by normalized email, and auth confirm verify keys by public client actor plus OTP type without using the token hash as a rate-limit key. Additional broader public-auth limits still need a durable/cache-backed storage decision. |
