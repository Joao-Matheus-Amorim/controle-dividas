...
| --- | --- | --- |
| Public auth | `auth.signup.authorized_email.check`; login, signup, password reset still pending | Highest abuse risk; actor may be anonymous. The first runtime boundary keys the signup authorized email preflight by normalized email with organization key `public-auth`; broader public-auth limits still need a durable/cache-backed storage decision. |
