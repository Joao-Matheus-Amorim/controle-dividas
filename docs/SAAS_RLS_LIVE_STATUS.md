...
- billing portal runtime usa Stripe Billing Portal Session para owner/admin da organizacao resolvida no servidor com `stripe_customer_id` e rate limit `billing.portal.start`;
...
- webhook Stripe, assinatura sincronizada e enforcement comercial, com runtime bloqueado ate a evidencia real de checkout e portal exigida por `docs/audits/BILLING_WEBHOOK_RUNTIME_CONTRACT.md`;
