...
- admin invitation audit runtime e admin invitation rate limit runtime versionados em `app/protected/admin/invitation-actions.ts` para criar, revogar e preparar reenvio e em `app/auth/convite/actions.ts` para aceitar convite via `record_audit_event` e `lib/security/sensitive-rate-limit.ts`, sem email delivery, UI ou remocao de `ADMIN_EMAIL`;
