...
- Runtime parcial de convite/admin esta versionado em `app/protected/admin/invitation-actions.ts` para criar, revogar, preparar reenvio e compensar delivery falho; delivery adapter server-only esta versionado em `lib/admin-invitations/delivery.ts`; aceite/linking esta versionado em `supabase/migrations/045_accept_admin_invitation_rpc.sql` e `app/auth/convite/actions.ts`; UI de aceite esta versionada em `app/auth/convite/page.tsx` e `components/admin-invitation-acceptance-form.tsx`; contrato delivery/UI esta versionado em `docs/audits/ADMIN_INVITATION_DELIVERY_UI_CONTRACT.md`; cron de expiracao e remocao de `ADMIN_EMAIL` continuam fora deste passo.
...
- admin invitation audit runtime e admin invitation rate limit runtime versionados em `app/protected/admin/invitation-actions.ts` para criar, revogar, preparar reenvio e compensar delivery falho, delivery adapter server-only em `lib/admin-invitations/delivery.ts`, aceite de convite em `app/auth/convite/actions.ts` via `record_audit_event` e `lib/security/sensitive-rate-limit.ts`, e UI de aceite em `app/auth/convite/page.tsx` + `components/admin-invitation-acceptance-form.tsx`, sem cron ou remocao de `ADMIN_EMAIL`;
