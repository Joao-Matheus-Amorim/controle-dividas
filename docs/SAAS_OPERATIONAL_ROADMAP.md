...
- receivable income audit runtime versionado em `app/protected/contas-a-receber/actions.ts` para `finance.receivable.status.update` e `finance.receivable.delete` via `record_audit_event`;
- expense audit runtime versionado em `app/protected/gastos/actions.ts` para `finance.expense.delete` via `record_audit_event`;
- bank audit runtime versionado em `app/protected/bancos/actions.ts` para `finance.bank.balance.update` e `finance.bank.delete` via `record_audit_event`;
...
- limites explicitos: sem category-delete audit logging runtime, rate limit runtime, data retention runtime, UI, billing ou E2E neste passo;
...
- implementar category-delete audit logging em PR dedicado antes de declarar a familia de audit logging financeiro completa;
