...
- receivable income audit runtime, receivable delete rate limit runtime e receivable status rate limit runtime versionados em `app/protected/contas-a-receber/actions.ts` para `finance.receivable.status.update` e `finance.receivable.delete` via `record_audit_event` e `lib/security/sensitive-rate-limit.ts`;
...
- bank audit runtime, bank delete rate limit runtime e bank balance rate limit runtime versionados em `app/protected/bancos/actions.ts` para `finance.bank.balance.update` e `finance.bank.delete` via `record_audit_event` e `lib/security/sensitive-rate-limit.ts`;
