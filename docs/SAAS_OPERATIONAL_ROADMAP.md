...
- Runtime parcial de convite/admin esta versionado em `app/protected/admin/invitation-actions.ts` para criar, revogar e preparar reenvio, com audit events e rate limit; aceite/linking, email delivery, cron de expiracao e remocao de `ADMIN_EMAIL` continuam fora deste passo.
...
- admin invitation audit runtime e admin invitation rate limit runtime versionados em `app/protected/admin/invitation-actions.ts` para criar, revogar e preparar reenvio de convite via `record_audit_event` e `lib/security/sensitive-rate-limit.ts`, sem email delivery, aceite/linking, UI ou remocao de `ADMIN_EMAIL`;
