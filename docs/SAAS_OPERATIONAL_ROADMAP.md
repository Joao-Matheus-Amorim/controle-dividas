...
- payable bill audit runtime, payable delete rate limit runtime e payable status rate limit runtime versionados em `app/protected/contas-a-pagar/actions.ts` para `finance.payable.status.update` e `finance.payable.delete` via `record_audit_event` e `lib/security/sensitive-rate-limit.ts`;
